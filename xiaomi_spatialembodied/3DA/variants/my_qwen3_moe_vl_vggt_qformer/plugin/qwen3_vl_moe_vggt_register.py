"""
MS-Swift custom registration file for Qwen3-VL-30B-A3B MoE + VGGT Q-Former.

This file is meant to be passed via ``--custom_register_path``.
It registers:
- a template that prepares ``image_tchw`` for VGGT (with temporal repeat for Q-Former)
- a model type pointing to the MoE checkpoint directory

Project dir: .../my_qwen3_moe_vl_vggt_qformer
"""
...
_PROJECT_DIR = _PLUGIN_DIR.parent  # .../my_qwen3_moe_vl_vggt_qformer
...
def _preprocess_image_to_vggt_tensor(x: Union[str, Image.Image]) -> torch.Tensor:
    """Aspect-ratio resize + pad to 518x518 (matches VGGT ``mode="pad"``)."""
...
def _make_image_tchw(
    ...
    *,
    ...
    """
    Build VGGT input stream with temporal repeat for Q-Former compatibility.
    Output: [N * temporal_patch_size, C, 518, 518] float in [0,1].
    """
    if not images:
    ...
    return images_ntchw.reshape(
        -1, images_ntchw.shape[2], images_ntchw.shape[3], images_ntchw.shape[4]
    ).contiguous()
...
def _grid_hw_all_equal(media_grid_thw: torch.Tensor) -> bool:
    ...
    for i in range(1, int(media_grid_thw.shape[0])):
        if int(media_grid_thw[i, 1].item()) != h0 or int(media_grid_thw[i, 2].item()) != w0:
            return False
    return True
    ...
    *,
    ...
    for i, x in enumerate(images):
        ...
        aligned_images.append(
            _resize_image_to_target_grid(x, target_h=target_h, target_w=target_w, patch_size=patch_size)
        )
...
def _decode_video_frames(video: Any, *, num_frames: int) -> List[Image.Image]:
    if num_frames <= 0:
    ...
    if isinstance(video, torch.Tensor):
        x = video.detach().cpu()
        if x.ndim != 4:
            raise ValueError(f"Video tensor must be 4D, got shape {tuple(x.shape)}")
        if x.shape[-1] == 3:
            thwc = x
        elif x.shape[1] == 3:
            thwc = x.permute(0, 2, 3, 1).contiguous()
        else:
            raise ValueError(f"Cannot infer channel dim: shape {tuple(x.shape)}")
...
# Template
# ==============================================================================
class Qwen3VLMoeVGGTQFormerTemplate(Qwen3VLTemplate):
    version = "v3"
    images_input = None
    videos_input = None
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        num_query_tokens = int(getattr(self.config, "vggt_connector_config", {}).get("num_query_tokens", 8))
            ...
            if mm_data:
                if media_type == "images":
                    ...
                    aligned_images, media_inputs, media_grid_thw = _align_images_to_same_grid_if_needed(
                        images=list(mm_data), image_processor=processor.image_processor,
                    )
                    ...
                    video_tchw = _make_video_tchw(
                        mm_data, video_grid_thw=media_grid_thw,
                        temporal_patch_size=int(self.config.vision_config.temporal_patch_size),
                    )
                ...
                def _get_new_tokens(i):
                    if media_type == "images":
                        ...
                        return [media_token] * (num_query_tokens + int(token_len))
                    return [media_token] * num_query_tokens + splited_tokens[i]
        ...
        if kwargs.get("video_tchw") is None and self.videos_input:
            ...
            if video_grid_thw is None:
                media_inputs = self.processor(
                    ...
                    videos=self.videos_input, return_tensors="pt", do_resize=False,
                )
                video_grid_thw = media_inputs["video_grid_thw"]
            kwargs["video_tchw"] = [
                _make_video_tchw(
                    self.videos_input, video_grid_thw=video_grid_thw,
                    temporal_patch_size=int(self.config.vision_config.temporal_patch_size),
                )
            ]
...
# Loader
# ==============================================================================
class Qwen3VLMoeVGGTQFormerLoader(Qwen3VLMoeLoader):
...
register_template(QwenTemplateMeta(
    "qwen3_vl_moe_vggt_qformer",
    template_cls=Qwen3VLMoeVGGTQFormerTemplate,
    default_system=None,
))
...
register_model(
    ModelMeta(
        ...
        Qwen3VLMoeVGGTQFormerLoader,
        template="qwen3_vl_moe_vggt_qformer",
