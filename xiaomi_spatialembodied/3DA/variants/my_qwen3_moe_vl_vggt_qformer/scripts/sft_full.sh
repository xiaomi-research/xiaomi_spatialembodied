...
#
# Minimal single-file demo for the Q-Former connector variant.
#
...
swift sft \
  --model "${BASE_CKPT:-/path/to/checkpoints/qwen3_moe_vggt_qformer_base_ckpt}" \
  --model_type qwen3_vl_moe_vggt_qformer \
  --custom_register_path xiaomi_spatialembodied/3DA/variants/my_qwen3_moe_vl_vggt_qformer/plugin/qwen3_vl_moe_vggt_register.py \
  ...
  --max_steps 10 \
  --max_length 16096 \
  --output_dir "${OUTPUT_DIR:-/path/to/workspace/sft_checkpoint/qwen3_moe_vggt_qformer/test_zero3}" \
  --run_name test_zero3_moe_qformer \
  --deepspeed zero3 \
