"""
MS-Swift custom registration file for Qwen3-VL-30B-A3B + VGGT.
...
_PROJECT_DIR = _PLUGIN_DIR.parent  # .../my_qwen3_vggt_xattn_mlp
# This project lives at:
#   ms-swift/examples/custom/4d-intelligence/my_qwen3_vggt_xattn_mlp/plugin/...
# so parents[3] is `.../examples/custom`.
...
# ==============================================================================
# VGGT / Qwen grid alignment knobs
# ==============================================================================
# These two settings control how the VGGT patch grid is mapped onto the Qwen3 patch grid.
# The defaults reproduce the released checkpoints (letterbox + bilinear); changing either
# one invalidates weights trained with the other setting. See "VGGT / Qwen grid alignment"
# in `xiaomi_spatialembodied/3DA/README.md`.
_SUPPORTED_VGGT_INTERP_MODES = ("bilinear", "nearest-exact", "nearest", "bicubic")


def _resolve_letterbox(config: Any) -> bool:
    """
    Whether the VGGT input stream is letterboxed to a white 518x518 square.

    Precedence: ``vggt_connector_config["letterbox"]`` (persisted in the checkpoint)
    > ``VGGT_LETTERBOX`` env var > True (released-checkpoint behaviour).
    """
    connector_cfg = getattr(config, "vggt_connector_config", None) or {}
    if "letterbox" in connector_cfg:
        return bool(connector_cfg["letterbox"])
    return os.getenv("VGGT_LETTERBOX", "1").strip().lower() not in {"0", "false", "no", "off"}


def _resolve_interp_mode(config: Any) -> str:
    """
    Resampling mode used to map the VGGT patch grid onto the Qwen3 patch grid.

    Precedence: ``vggt_connector_config["interp_mode"]`` > ``VGGT_INTERP_MODE`` env var
    > "bilinear" (released-checkpoint behaviour).
    """
    connector_cfg = getattr(config, "vggt_connector_config", None) or {}
    mode = str(connector_cfg.get("interp_mode") or os.getenv("VGGT_INTERP_MODE", "bilinear"))
    if mode not in _SUPPORTED_VGGT_INTERP_MODES:
        raise ValueError(
            f"Unsupported VGGT_INTERP_MODE / interp_mode={mode!r}; "
            f"expected one of {_SUPPORTED_VGGT_INTERP_MODES}."
        )
    return mode
...
def _to_pil_rgb(x: Union[str, Image.Image]) -> Image.Image:
    ...
    if isinstance(x, str):
        ...
        # `x.filename`. On modern Pillow (checked on 12.0.0) `convert()` and `resize()`
        # clear `filename`, so re-opening is a no-op on the MS-Swift path -- images there
        # have already been through `qwen_vl_utils.fetch_image`. It only fires when a
        # caller hands us an image that still carries a filename (e.g. `deepcopy` of an
        # `Image.open(...)`, or an older Pillow that propagated `__dict__`), and in that
        # case re-opening would silently feed the VGGT stream the on-disk pixels instead
        # of the in-memory ones the Qwen stream sees.
        img = x
    else:
        raise TypeError(f"Unsupported image type for VGGT preprocessing: {type(x)}")
    # If there's an alpha channel, blend onto white background (match VGGT helper behavior).
    if img.mode == "RGBA":
...
def _preprocess_image_to_vggt_tensor(
    x: Union[str, Image.Image],
    *,
    letterbox: bool = True,
) -> torch.Tensor:
    """
    Preprocess one image (path or PIL) to a VGGT-compatible tensor:
    - resize keeping the aspect ratio (long side 518, short side rounded to a multiple of 14)
    - if `letterbox` is True, additionally pad to a white 518x518 square
    - output: (3, H, W) float in [0,1]

    `letterbox=True` reproduces the released checkpoints. Note that padding to a square
    while the Qwen stream keeps the unpadded aspect ratio makes the two patch grids agree
    only near the image centre -- see "VGGT / Qwen grid alignment" in `3DA/README.md`.
    """
    from torchvision import transforms as TF  # runtime import (same dependency as VGGT)

    target_size = 518
    patch_size = 14  # VGGT patch size
    ...
    if width >= height:
        ...
        # `max(patch_size, ...)` guards against extremely elongated images rounding to 0.
        new_height = max(patch_size, round(height * (new_width / width) / patch_size) * patch_size)
    else:
        new_height = target_size
        new_width = max(patch_size, round(width * (new_height / height) / patch_size) * patch_size)

    img = img.resize((new_width, new_height), Image.Resampling.BICUBIC)
    img = to_tensor(img)  # (3,H,W) float in [0,1]

    if letterbox:
        # Pad to square 518x518 with white (value=1.0)
...
def _make_image_tchw(
    ...
    *,
    ...
    """
    Build the VGGT input stream `image_tchw`, supporting both `str` paths and
    `PIL.Image` objects (ms-swift may pre-load images).

    With `letterbox=True` every image is a 518x518 square, so the stack is trivial.
    With `letterbox=False` the per-image shapes follow each image's aspect ratio; if a
    sample contains differently shaped images they are padded to the common maximum
    (rounded up to a multiple of 14) so they can still be stacked. Samples whose images
    were aligned by `_align_images_to_same_grid_if_needed` share a shape and are not padded.

    Output is float32 in [0,1], shape [T,C,H,W] where T = temporal_patch_size * N_images.
    """
    if not images:
        raise ValueError("images is empty for VGGT preprocessing")

    images_nchw: List[torch.Tensor] = [
        _preprocess_image_to_vggt_tensor(x, letterbox=letterbox) for x in images
    ]

    if not letterbox and len({tuple(t.shape) for t in images_nchw}) > 1:
        vggt_patch_size = 14
        h_max = max(t.shape[1] for t in images_nchw)
        w_max = max(t.shape[2] for t in images_nchw)
        h_max = -(-h_max // vggt_patch_size) * vggt_patch_size
        w_max = -(-w_max // vggt_patch_size) * vggt_patch_size
        images_nchw = [
            torch.nn.functional.pad(
                t,
                (0, w_max - t.shape[2], 0, h_max - t.shape[1]),
                mode="constant",
                value=1.0,
            )
            if tuple(t.shape) != (3, h_max, w_max)
            else t
            for t in images_nchw
        ]

    images_nchw = torch.stack(images_nchw, dim=0)

    # Repeat each image along the temporal dimension to match Qwen3 temporal_patch_size.
    images_ntchw = images_nchw.unsqueeze(1).repeat(1, int(temporal_patch_size), 1, 1, 1)  # (N,T,C,H,W)
...
def _align_images_to_same_grid_if_needed(
    *,
    ...
    # `do_resize=False` makes the image processor reshape the raw pixels into patches, which
    ...
    # Read sizes without decoding the pixels: `_to_pil_rgb` would force a full decode of
    # every image on every sample encode.
    misaligned = []
...
def _decode_video_frames(
    ...
    *,
    ...
    """
    Decode a video into a list of PIL frames (RGB).

    Supported input types:
    - str: local video path (decoded via decord; only used as a fallback)
    - torch.Tensor: decoded frames (common output of `qwen_vl_utils.fetch_video`)
    - list[PIL.Image]: already-decoded frames
    """
    if num_frames <= 0:
        raise ValueError(f"num_frames must be > 0, got {num_frames}")

    # decoded tensor frames (common for Qwen3-VL v3 in ms-swift)
    if isinstance(video, torch.Tensor):
        x = video.detach().cpu()
        if x.ndim != 4:
            raise ValueError(f"Video tensor must be 4D (T,H,W,C) or (T,C,H,W), got shape {tuple(x.shape)}")
        # normalize to THWC uint8
        if x.shape[-1] == 3:
            thwc = x
        elif x.shape[1] == 3:
        ...
        if thwc.dtype != torch.uint8:
            # Assume float in [0,1] (the common case); if not, this still produces a valid uint8 image.
    ...
    if isinstance(video, str):
        from decord import VideoReader, cpu  # runtime import
...
def _make_video_tchw(
    ...
    *,
    ...
    """
    Build VGGT input stream `video_tchw` by:
    - sampling frames from each video (uniformly)
    - applying the same VGGT image preprocessing (pad to 518x518)

    Output:
      - float tensor in [0,1], shape [T,C,518,518]
      - T matches Qwen3 temporal patching: sum_i (video_grid_thw[i,0] * temporal_patch_size)
    """
    if not videos:
    ...
    for i, v in enumerate(videos):
        ...
        # Qwen3 vision uses Conv3d with temporal_patch_size, so input frames should be grid_t * temporal_patch_size.
...
class Qwen3VLVGGTTemplate(Qwen3VLTemplate):
        ...
        for media_type in ["images", "videos"]:
            ...
            if mm_data:
                if media_type == "images":
                    ...
                    # Save for generate(); training should rely on per-sample fields, not template state.
                    self.images_input = aligned_images
                    # Spatial-MLLM style: generate image_tchw at sample encode time and carry it in the sample dict.
                    image_tchw = _make_image_tchw(
                        aligned_images,
                        temporal_patch_size=int(self.config.vision_config.temporal_patch_size),
                        letterbox=_resolve_letterbox(self.config),
                    )
                    # IMPORTANT: wrap in a list, otherwise gather_list() will iterate the Tensor along dim0.
                    ...
                    # VGGT video stream: sample frames then reuse the same VGGT image preprocessing.
    ...
    def _post_encode(self, model, inputs: Dict[str, Any]) -> Dict[str, Any]:
        # No-op for training: image_tchw is produced at sample encode time (Spatial-MLLM style).
        return inputs
    ...
    def generate(self, model, *args, **kwargs):
        # If callers already provided VGGT streams (typical for `swift infer` with datasets),
        # do not overwrite them with template state.
        ...
        if kwargs.get("video_tchw") is None and self.videos_input:
            # Prefer using the already-encoded `video_grid_thw` when available.
            video_grid_thw = kwargs.get("video_grid_thw", None)
            if video_grid_thw is None:
                # As a fallback (interactive usage), re-run processor to obtain `video_grid_thw`.
                # NOTE: this may differ from encode-time settings if custom mm_processor_kwargs were used.
...
class Qwen3VLMoeVGGTLoader(Qwen3VLMoeLoader):
    ...
    def get_model(self, model_dir: str, config, processor, model_kwargs):
        ...
        # DDP note:
        # - DataParallel (DDP) expects ONE full model replica per process/GPU.
        # - `device_map="auto"` will shard the model across multiple GPUs inside one process and may
        #   conflict with DDP (and also hit accelerate tied-weights edge cases).
        # So when distributed is enabled, force-disable device_map sharding.
        ...
        env = _get_vggt_env()
        # Let runtime env vars override config.json, so training/infer scripts can control VGGT placement/dtype.
        # This is important under DeepSpeed/torch_dtype where VGGT params may be cast by the engine.
        ...
        config.vggt_spatial_config = getattr(config, "vggt_spatial_config", None) or {
            # NOTE: VGGT original preprocessing helper uses 518px by default.
            "img_size": 518,
            "patch_size": 14,
            "embed_dim": 1024,
        ...
        # Persist the grid-alignment knobs into the config so that the template (which builds
        # the VGGT stream) and the model (which resamples it) can never disagree, and so that
        # the choice travels with a saved checkpoint. Existing keys are never overwritten.
        config.vggt_connector_config.setdefault("letterbox", _resolve_letterbox(config))
        config.vggt_connector_config.setdefault("interp_mode", _resolve_interp_mode(config))
