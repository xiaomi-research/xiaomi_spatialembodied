...
CUSTOM_MODEL_CFG = {
    ...
    "register_file": os.path.join(os.path.dirname(__file__), "3DA", "my_qwen3_vggt_xattnv2", "plugin", "qwen3_vl_moe_vggt_register.py"),
}
...
INFER_CFG = {
    ...
    # <-- UPDATE: set to your test image path. The shipped image is a synthetic
    # placeholder containing no third-party imagery. -->
    "image_paths": [os.path.join(os.path.dirname(__file__), "..", "demo", "sample.jpg")],
}
...
def init_engine() -> TransformersEngine:
    ...
    if not os.path.isdir(model_path) and not model_path.startswith("Qwen/"):
        raise FileNotFoundError(
            ...
            "Set CUSTOM_MODEL_CFG['model_path'] at the top of this file to either:\n"
            "  * the fused checkpoint you downloaded from Kaggle, or\n"
            "  * a local directory built with\n"
            "    xiaomi_spatialembodied/3DA/my_qwen3_vggt_xattnv2/plugin/save_qwen3_vl_moe_vggt.py\n"
            "Merely pointing at a bare Qwen3-VL base model will not work: the VGGT\n"
            "weights must already be merged into the checkpoint."
        )
